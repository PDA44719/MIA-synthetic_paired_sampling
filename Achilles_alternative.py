...
from src.shadow_data import create_shadow_training_data_membership_fixed_reference, create_shadow_training_data_membership_no_reference
...
parser.add_argument("--fixed_reference", type = bool, default = False,
                    help = "whether fixed reference paired sampling is used (True) or no reference paired sampling (False)")
...
def main():
    ...
    if FIXED_REFERENCE:
        datasets_train, labels_train, datasets_train_utility = create_shadow_training_data_membership_fixed_reference(df = df_aux, meta_data = meta_data,
        ...
        datasets_test, labels_test, _ = create_shadow_training_data_membership_fixed_reference(df = df_test, meta_data = meta_data,
                                    target_record = target_record, generator = generator, n_original = N_ORIGINAL,
                                    n_synth = N_SYNTHETIC, n_pos = N_POS_TEST, seeds = test_seeds)
    else:
        datasets_train, labels_train, datasets_train_utility = create_shadow_training_data_membership_no_reference(df = df_aux, meta_data = meta_data,
                                        target_record = target_record, generator = generator, n_original = N_ORIGINAL,
                                        n_synth = N_SYNTHETIC, n_pos = N_POS_TRAIN, seeds = train_seeds)
        test_seeds = list(range(N_POS_TRAIN * 2, N_POS_TRAIN * 2 + N_POS_TEST * 2)) # make it non overlapping
        datasets_test, labels_test, _ = create_shadow_training_data_membership_no_reference(df = df_test, meta_data = meta_data,
                                    target_record = target_record, generator = generator, n_original = N_ORIGINAL,
                                    n_synth = N_SYNTHETIC, n_pos = N_POS_TEST, seeds = test_seeds)
    enablePrint()
    ...
    if FIXED_REFERENCE:
        NAIVE_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}record_{SEED}seed_naive_fixed_reference.csv'
    else:
        NAIVE_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}record_{SEED}seed_naive_no_reference.csv'
    ...
    if PAIRED_SAMPLING:
        QUERY_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}record_{SEED}seed_query_fixed_reference.csv'
    else:
        QUERY_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}record_{SEED}seed_query_no_reference.csv'
    ...
    if PAIRED_SAMPLING:
        TARGETATTENTION_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}_{SEED}seed_targetattention_fixed_reference.csv'
    else:
        TARGETATTENTION_OUTPUT_PATH = f'{OUTPUT_DIR}/output_{TARGET_RECORD_ID}_{SEED}seed_targetattention_no_reference.csv'
